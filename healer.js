...
async function returnToFarm(farm){
   ...
   async function attackMode (targ){
       ...
       attack(targ)}
           ...
           move
           (
           character.real_x+(targ.real_x-character.real_x)+70,
           character.real_y+(targ.real_y-character.real_y)+70
           )
       }else if (can_attack(targ)){
           attack(targ)
       }}
	
//setTimeout(partyHeal(currentParty),1000))

partyHeal(currentParty)

async function partyHeal (party){
	for (let member of party){
		let healTarg = get_player(member)
		change_target(healTarg)
		if(!is_on_cooldown("heal") && healTarg.hp < healTarg.max_hp / 1.5){
			if(!is_in_range(healTarg,"heal")){
			   smart_move(healTarg)}else{ await use_skill("heal",healTarg)}}}}
 ...
 //HP und MP Management Funktionen
   ...
   async function useMpPot(){
       if(charMp < charMaxMp * 0.30 && charMp != charMaxMp){
       use_hp_or_mp("use_mp")
       await sleep(100)}}
